...
'
    ...
    test_expect_success HAVE_NEWFSTATAT \
        "magic /dev/sydbox boolean checking works with -y write:deny [memory_access:${magic_mem_access}]" '
    syd \
        --memaccess '${magic_mem_access}' \
...
'
    ...
    test_expect_success \
        "magic /dev/sydbox boolean checking works with -y switch [memory_access:${magic_mem_access}]" '
    syd \
        --memaccess '${magic_mem_access}' \
        -y core/sandbox/write:deny -- sh <<-\EOF
test -e /dev/sydbox/core/sandbox/write"?"
EOF
'
